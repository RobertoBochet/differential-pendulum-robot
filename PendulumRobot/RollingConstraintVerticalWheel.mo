...
model RollingConstraintVerticalWheel "Rolling constraint for wheel that is always perpendicular to x-y plane"
  ...
  Modelica.Mechanics.MultiBody.Interfaces.Frame_a frame_a "Frame fixed in wheel center point. x-Axis: upwards, y-axis: along wheel axis" annotation(
    Placement(transformation(extent = {{-16, 4}, {16, 36}}), iconTransformation(extent = {{-16, 4}, {16, 36}})));
  parameter Modelica.SIunits.Radius radius "Wheel radius";
  parameter Boolean lateralSlidingConstraint = true "= true, if lateral sliding constraint taken into account, = false if lateral force = 0 (needed to avoid overconstraining if two ideal rolling wheels are connect on one axis)" annotation(
    choices(checkBox = true),
    Evaluate = true);
  // Contact force
  Modelica.SIunits.Force f_wheel_0[3] "Contact force acting on wheel, resolved in world frame";
  Modelica.SIunits.Force f_lat "Contact force acting on wheel in lateral direction";
  Modelica.SIunits.Force f_long "Contact force acting on wheel in longitudinal direction";
  Modelica.SIunits.Force f_vert;
protected
  Real e_axis_0[3] "Unit vector along wheel axis, resolved in world frame";
  Modelica.SIunits.Position rContact_0[3] "Distance vector from wheel center to contact point, resolved in world frame";
  ...
  Modelica.SIunits.AngularVelocity w_0[3] "Angular velocity of wheel, resolved in world frame";
...
// Two non-holonomic constraint equations on velocity level (ideal rolling, no slippage)
  ...
  0 = v_0 * e_n_0;
//radius = frame_a.r_0*e_n_0;
  if lateralSlidingConstraint then
    0 = vContact_0 * e_lat_0;
    f_wheel_0 = f_lat * e_lat_0 + f_long * e_long_0 + f_vert * e_n_0;
  else
    0 = f_lat;
    f_wheel_0 = f_long * e_long_0 + f_vert * e_n_0;
  end if;
